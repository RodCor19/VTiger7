...
<script type="text/javascript">
  ...
  function graficar(){
      ...
      setupPivot({json: chartData, fields: field_definitions, filters: {}, rowLabels:['Campaña', 'Desde', 'Hasta',
               'SMS Enviados', 'Mensajes Enviados', '% Mails Abiertos','Cantidad Rebotados','Clientes activos por día','$ por dia',
  ...
  function setupPivot(input){
    input.callbacks = {afterUpdateResults: function(){
      ...
      jQuery('.row-labelable:checked').each(function(index){
          ...
          }else if(colName=='Clientes activos por día' || colName== "Clientes reclutados por día" || colName=="% Descuento" || colName=="$ por dia" || colName=="$ por cliente"){
            var column = { 
             sClass: 'color3'
            };
  ...
  function actualizar(){
    ...
    jQuery.ajax({
           async: false,
           data: {'createdtime':createdtime,'rango':edad_str,'sexo':sexo_str,'canal':canal_str,'programa':programa_str,'estatuto':estatuto_str,'tipo':tipo_str,'familia':familia, 'rubro':rubro, 'localizacion': localizacion, 'adherido':adherido, 'nombreLocal':nombreLocal, 'formapago':formapago},
