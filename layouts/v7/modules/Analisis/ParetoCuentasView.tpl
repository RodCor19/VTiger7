...
function  getChartRelatedData() {
  ...
  var filtrosExcel={'createdtime':"Fecha",'familias':"Familias",'rubro':"Rubros",'localizacion':"Localización",'adherido':"Adherido",'local':"Local",'vista':"Vista",'m2':"m2"};
  ...
  function graficarTabla (chartData)
{
    ...
    if (chartData.length > 0)
    {
        ...
        if(vista!="M2"){
          $cabecera.append('<th style="color:#0070BA">m2</th>');  
        }
    ...
    if (chartData.length > 0)
    {
        ...
        for (var i = 0, l = chartData.length; i < l-1; i++)
        {
            ...
            if(vista!="M2"){
              fant += '<td>' + formatoMoneda(chartData[i][3]) + '</td>';
            }
          fant += '</tr><tr>';
        }
