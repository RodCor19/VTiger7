...
<div class="detailViewContainer">
  <div class="row-fluid detailViewTitle"  style="padding-bottom:10px;"><a href="index.php?module=Analisis&view=Deudores">Deudores</a>
    <span class="recordLabel font-x-x-large textOverflowEllipsis span pushDown" title="gika"><span class="">Morosidad por Cliente</span>&nbsp;</span>
  ...
  <div class="detailViewInfo row-fluid">
    ...
    <div class="detailViewInfo row-fluid">
      ...
      <span class="span3">
        <select multiple class="select2" style="width: 85%" name="local" id="local" onChange="javascript:actualizar();" >
 <!--         <option value="">{vtranslate('LBL_ALL', $MODULE_NAME)}</option>-->
          {foreach item=local from=$locales}
          <option value="{$local[1]}" {if $local[1]== $idLocal}selected=""{/if}>
              {$local[0]}
            </option>
          {/foreach}
        </select>
...
function pushIfNew(obj) {
  ...
  $( document ).ready(function() {
    ...
    field_definitions = [
          ...
          {name: 'LUC',   type: 'int',   filterable: true, rowLabelable: true},
          ...
          {name: '% Morosidad',   type: 'float',    filterable: true, rowLabelable: true,displayFunction: function(value){ return accounting.formatMoney(value,{ symbol: "%",  format: "%v %s", decimal:decimalSep,thousand:milSep,precision :0 })}},
          ...
          {name: 'Rotación(Meses)',   type: 'float',    filterable: true, rowLabelable: true,displayFunction: function(value){ return accounting.formatNumber(value,1,milSep,decimalSep)}},
          {name: 'Deuda Vencida(Cliente/Total)',   type: 'float',    filterable: true, rowLabelable: true,displayFunction: function(value){ return accounting.formatMoney(value,{ symbol: "%",  format: "%v %s", decimal:decimalSep,thousand:milSep,precision :1 })}},

          {name: 'Acumulado',   type: 'float',    filterable: true, rowLabelable: true,displayFunction: function(value){ return accounting.formatMoney(value,{ symbol: "%",  format: "%v %s", decimal:decimalSep,thousand:milSep,precision :1 })}}, 
        {name: 'Orden', type: 'int', filterable: false, pseudo: true,
            pseudoFunction: function(row){
              var ret="";
              switch (row["Nombre Fantasía"]) {
                  case "TOTAL":
                      ret = 1;
                      break;
  ...
  function graficar(){
    ...
    filas=["Nro","Nombre Fantasía","Razón Social","LUC","Deuda a Vencer",
        "Deuda Vencida","Total Deuda","% Morosidad","Rotación(Meses)","Deuda Vencida(Cliente/Total)","Acumulado","Orden"];
            nroOrden=11;
            ordenNumerico=[0,3,4,5,6,7,8,9,10];
      ...
      local_ant="";      
       oTable = jQuery('#results > table').dataTable({
        ...
        "aoColumnDefs": [
            ...
            { "sTitle": "Rotación <br>  (meses)", "aTargets": [ 8 ] },
            { "sTitle": "Deuda vencida <br> (cliente/total)", "aTargets": [ 9 ] }
        ],
         // "aoColumns": aoColumns
         "drawCallback": function( settings ) {
          jQuery('#pivot-demo tr:eq(0) th:eq(0)').text("Text update by code");
        },
         "fnRowCallback": function( row, data, index ) {
            ...
            var indexx = (page * length + (index +1));
            jQuery('td:eq(0)', row).html(indexx);
  ...
  function actualizar(){
    ...
    var createdtime = {};
    //createdtime.start = dateRangeValComponents[dateRangeVal];
    createdtime.start = '';
    createdtime.end = dateRangeVal;
    ...
    jQuery.ajax({
           ...
           url:  'index.php?module=Analisis&action=Morosidad',
           dataType:"json",
