	...
	public function getAjax(Vtiger_Request $request){	
		...
		//if($acceso){
			...
			$query="SELECT t.ticketid as id, t.status AS estado, t.priority AS prioridad, t.title, CONCAT(cd.firstname,' ',cd.lastname) AS 			contacto, cf.cf_1130 AS fecha, u.user_name AS 'Asignado a', a.accountname as local, lpcategoria 	as tipo, category as categoria
				FROM vtiger_troubletickets t JOIN vtiger_crmentity c ON t.ticketid = c.crmid AND c.deleted = 0 LEFT JOIN vtiger_contactdetails cd ON t.contact_id = cd.contactid JOIN vtiger_users u ON u.id = smcreatorid LEFT JOIN vtiger_account a ON t.parent_id = a.accountid JOIN vtiger_ticketcf cf ON t.ticketid = cf.ticketid";
