...
class Analisis_Filtros_View extends Vtiger_Index_View {
	...
	public function getFiltros($request){
		...
		$resultSet = $adb->pquery("SELECT * FROM lp_analisis_filtros WHERE ( usuario = ? OR publico = 1 ) AND tabla = ? ",array($idUser,$tabla));
	...
	*/
	...
	public function guardarFiltro(Vtiger_Request $request){
		...
		$existe = $adb->pquery("SELECT 1 FROM lp_analisis_filtros WHERE nombre = ? AND tabla = ? AND ( usuario = ? OR publico = 1 )",array($datos['nombre'],$datos['tabla'],$usuario));
	...
	public function eliminarFiltro($datos){
		$nombre  			= $datos['nombre'];
		$tabla  		 	= $datos['tabla'];
		$currentUserModel 	= Users_Record_Model::getCurrentUserModel();
		$usuario 			= $currentUserModel->getId();
		$adb 				= PearDatabase::getInstance();
		$existe = $adb->pquery("SELECT 1 FROM lp_analisis_filtros WHERE nombre = ? AND usuario = ? AND tabla = ?",array($nombre,$tabla,$usuario));
		if( $adb->num_rows($existe) ){
			$result = $adb->pquery("DELETE FROM lp_analisis_filtros WHERE nombre = ? AND usuario = ? AND tabla = ?");
			return array('success' => true, 'message' => 'Filtro eliminado correctamente');
		}else{
			return array('success' => false,'message' => "No tiene los privilegios suficientes para eliminar este filtro");
		}
	...
	public function modificarFiltro($datos){
		...
		if( $adb->num_rows($existe) ){
			...
			return array('success' => false,'message' => "No tiene los privilegios suficientes para modificar este filtro");
		}
	}
	public function crearFiltro($datos){
		...
		$filtros = $this->filtrosToDb($datos['filtros']);
		$tabla = $datos['tabla'];
		$publico = $datos['publico'] == 'true' | 0;
		$params = array($id,$nombre,$rows,$cols,$filtros,$usuario,$tabla,$publico);		
		$result = $adb->pquery("INSERT INTO lp_analisis_filtros (id,nombre,filas,columnas,filtros,usuario,tabla,publico) 
			VALUES(?,?,?,?,?,?,?,?)",$params);
