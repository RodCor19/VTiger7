	...
	public function getQuery(Vtiger_Request $request){
		...
		if($vista!="detalle"){
			...
			SUM(if(vc.Aniomes>='".$fecha_desde."' AND vc.Aniomes <= '".$fecha_hasta."',vc.VentasSIva,0)) AS ventas,
				...
				(select SUM(if(cod_grupocon='ARRPORCENTUAL' AND ar.Aniomes>='".$fecha_desde."' AND ar.Aniomes <= '".$fecha_hasta."',1,0))FROM lp_arrendamientos ar WHERE ar.numcont_lin = ac.acnumerocontrato AND ar.Aniomes >= '".$fecha_desde_anterior."' AND ar.Aniomes <= '".$fecha_hasta."' ) AS arr_porc_cant,
			(
			SELECT SUM(ImpSIva) 
			...
			if($vista=="actual"){
				...
				$query.=" ,SUM(if(vc.Aniomes>='".$fecha_desde_anterior."' AND vc.Aniomes <= '".$fecha_hasta_anterior."',vc.VentasSIva,0)) AS ventas_ant,
					...
					(select SUM(if(cod_grupocon='ARRPORCENTUAL' AND ar.Aniomes>='".$fecha_desde_anterior."' AND ar.Aniomes <= '".$fecha_hasta_anterior."',1,0))FROM lp_arrendamientos ar WHERE ar.numcont_lin = ac.acnumerocontrato AND ar.Aniomes >= '".$fecha_desde_anterior."' AND ar.Aniomes <= '".$fecha_hasta."' ) AS arr_porc_ant_cant";	
			}	
		...
		if($vista!="detalle"){
			...
			while ($row = $adb->fetch_array($result)){
				...
				if(isset($row['arr_porc_ant_cant']) 
					...
					$arr_tot_mes_ant=round($arr_porcent_ant/$row['arr_porc_ant_cant'],2);

				$arr_tot_mes=0;
				if(isset($row['arr_porc_cant']) 
					&& $row['arr_porc_cant']!="")	
					$arr_tot_mes=round($arr_porcent/$row['arr_porc_cant'],2);
