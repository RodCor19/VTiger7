...
// agregar ipc a venta contable en vista actual
			...
			$query.=" FROM(
			SELECT lpfamilia,lprubro,s.metros,ac.accountid,superficieid,ac.acnumerocontrato,ac.accountname,s.fecha, IFNULL(
					(SELECT s1.fecha FROM vtiger_superficie s1 WHERE s.cuenta=s1.cuenta
						AND s.fecha < s1.fecha LIMIT 1
					),NOW()) AS fecha_prox, 
			IFNULL(s.arminimo,ac.acarminimo)		
			 as arr_min2,
			IF(s.arminimo is null or s.arminimo=0,ac.acarminimo,s.arminimo) as arr_min,createdtime
