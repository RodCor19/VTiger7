...
// venta con ipc
				$query.="(SELECT  SUM(venta *(1+(ipcactual-IFNULL(ipcmes,ipcactual))/100) ) AS ventas 
				FROM(SELECT Contrato,Aniomes, vc.VentasSIva AS venta, ipc.ipcmensual AS ipcmes,
				(SELECT ipcmensual FROM vtiger_ipc WHERE CONCAT(ipcanio,LPAD(ipcmes,2,'0')) >= '".$fecha_desde."' AND CONCAT(ipcanio,LPAD(ipcmes,2,'0')) <= '".$fecha_hasta."' ORDER BY ipcanio DESC, ipcmes DESC LIMIT 1) AS ipcactual
...
// agregar ipc a venta contable en vista actual
			...
			if ($conIpc == 'ipcNo'){
				...
				$query.="(SELECT  SUM(venta *(1+(ipcactual-IFNULL(ipcmes,ipcactual))/100) ) AS ventas 
				FROM(SELECT Contrato,Aniomes, vc.VentasSIva AS venta, ipc.ipcmensual AS ipcmes,
				(SELECT ipcmensual FROM vtiger_ipc WHERE CONCAT(ipcanio,LPAD(ipcmes,2,'0')) >= '".$fecha_desde_anterior."' AND CONCAT(ipcanio,LPAD(ipcmes,2,'0')) <= '".$fecha_hasta."' ORDER BY ipcanio DESC, ipcmes DESC LIMIT 1) AS ipcactual
				FROM lp_ventas_contratos vc
				LEFT JOIN vtiger_ipc ipc ON CONCAT(ipcanio,LPAD(ipcmes,2,'0')) = vc.Aniomes 
		...
		while ($row = $adb->fetch_array($result)){
			switch ($agrupar) {
				case 'familia':
					...
					$localesnombre[$cuenta]=$row["lpfamilia"];
					break;
				case 'rubro':
					$cuenta=$row["lprubro"];
					$locales[$cuenta]=$row["lprubro"];
					$localesnombre[$cuenta]=$row["lprubro"];
					break;
