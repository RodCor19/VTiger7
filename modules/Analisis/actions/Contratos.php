...
class Analisis_Contratos_Action extends Vtiger_BasicAjax_Action{
	...
	public function getQuery(Vtiger_Request $request){
		...
		if($vista == "actual" || $vista == "resumen" || $vista == "facturacion_detalle"){

			$filtro_fechas="";
			if($vista == 'resumen' || $vista == "facturacion_detalle"){
				if ($fecha_desde!=""){
			...
			$query.=" SUM(IF(ar.Aniomes>='".$fecha_desde."' AND ar.Aniomes <= '".$fecha_hasta."' AND 
				...
				";
			//agregado por VR - Facturacion 
			$query .= "SUM(IF(ar.Aniomes>='".$fecha_desde."' AND ar.Aniomes <= '".$fecha_hasta."' AND ar.AnioMes >=CONCAT(YEAR(fecha),LPAD(MONTH(fecha),2,'0')) 
				AND ar.AnioMes < CONCAT(YEAR(fecha_prox),LPAD(MONTH(fecha_prox),2,'0')),ar.ImpSIva,0 )) AS facturacion,";
				
			//agregado por VR, para la vista Facturacion detalle
			if($vista == "facturacion_detalle"){
				$query .= " SUM( IF( ar.Aniomes >= '".$fecha_desde."' AND ar.Aniomes <= '".$fecha_hasta."' AND cod_grupocon = 'ARRMINIMO' AND ar.AnioMes >= CONCAT( YEAR( fecha ), LPAD( MONTH( fecha ), 2, '0' )) AND ar.AnioMes < CONCAT( YEAR( fecha_prox ), LPAD( MONTH( fecha_prox ), 2, '0' )), ar.ImpSIva, 0 )) AS arr_minimo,";
				$query .= " SUM( IF( ar.Aniomes >= '".$fecha_desde."' AND ar.Aniomes <= '".$fecha_hasta."' AND cod_grupocon = 'FONDOPROM' AND ar.AnioMes >= CONCAT( YEAR( fecha ), LPAD( MONTH( fecha ), 2, '0' )) AND ar.AnioMes < CONCAT( YEAR( fecha_prox ), LPAD( MONTH( fecha_prox ), 2, '0' )), ar.ImpSIva, 0 )) AS fondo_prom,";
				$query .= " SUM( IF( ar.Aniomes >= '".$fecha_desde."' AND ar.Aniomes <= '".$fecha_hasta."' AND (cod_grupocon = 'GASTOSCOMUNES' OR cod_grupocon = 'GASTOSCOMUES') AND ar.AnioMes >= CONCAT( YEAR( fecha ), LPAD( MONTH( fecha ), 2, '0' )) AND ar.AnioMes < CONCAT( YEAR( fecha_prox ), LPAD( MONTH( fecha_prox ), 2, '0' )), ar.ImpSIva, 0 )) AS gastos_com,";
				$query .= " SUM( IF( ar.Aniomes >= '".$fecha_desde."' AND ar.Aniomes <= '".$fecha_hasta."' AND cod_grupocon = 'OTROS' AND ar.AnioMes >= CONCAT( YEAR( fecha ), LPAD( MONTH( fecha ), 2, '0' )) AND ar.AnioMes < CONCAT( YEAR( fecha_prox ), LPAD( MONTH( fecha_prox ), 2, '0' )), ar.ImpSIva, 0 )) AS otros,";
				$query .= " SUM( IF( ar.Aniomes >= '".$fecha_desde."' AND ar.Aniomes <= '".$fecha_hasta."' AND cod_grupocon = 'PAGOCTA' AND ar.AnioMes >= CONCAT( YEAR( fecha ), LPAD( MONTH( fecha ), 2, '0' )) AND ar.AnioMes < CONCAT( YEAR( fecha_prox ), LPAD( MONTH( fecha_prox ), 2, '0' )), ar.ImpSIva, 0 )) AS pago_cta,";
			}
...
// agregar ipc a venta contable en vista actual
			if($vista=="actual"){
				...
				//agregado por VR - Facturacion Anterior					
				$query .= "SUM(IF(ar.Aniomes>='".$fecha_desde_anterior."' AND ar.Aniomes <= '".$fecha_hasta_anterior."' AND ar.AnioMes >=CONCAT(YEAR(fecha),LPAD(MONTH(fecha),2,'0')) 
				AND ar.AnioMes < CONCAT(YEAR(fecha_prox),LPAD(MONTH(fecha_prox),2,'0')),ar.ImpSIva,0 )) AS facturacion_ant,";
			// si se quiere calcular la venta sin ipc
			if ($conIpc == 'ipcNo'){
...
//ac.acarminimo as arr_min,
			$query.=" FROM(
			SELECT lpfamilia,lprubro,s.metros,ac.accountid,superficieid,s.supnomcuenta AS accountname,s.supnumcontrato as acnumerocontrato,s.fecha, IFNULL((SELECT s1.fecha FROM vtiger_superficie s1 WHERE s.cuenta=s1.cuenta
			AND s.fecha < s1.fecha LIMIT 1),NOW()) AS fecha_prox, if(s.arminimo = '' OR s.arminimo is null,ac.acarminimo,s.arminimo ) AS arr_min,createdtime
			...
			GROUP BY acnumerocontrato,superficieid HAVING ventas > 0 ";
			...
			if($vista=="detalle"){
				...
				$query.="SELECT lpfamilia,lprubro,ac.accountid ,ac.accountname, ar.ImpSIva AS arr_porc ,
						...
						FROM lp_arrendamientos ar 
						INNER JOIN (SELECT lpfamilia,lprubro,s.metros,ac.accountid,superficieid,s.supnomcuenta as accountname,s.supnumcontrato as acnumerocontrato,s.fecha, 
				...
				$query.=" GROUP BY ac.acnumerocontrato, ac.superficieid, ar.AnioMes
	...
	public function procesarRespuesta($query,$vista,$request,$query_aux){
		...
		if($agrupar == "local") $query = str_replace("HAVING ventas > 0 ", "", $query);
		$result=$adb->query($query);
		$log->debug("Numero tuplas: ".$adb->num_rows($result));
		$result2=NULL;
		...
		if($agrupar!='local'){
			...
			$query_aux = str_replace("HAVING ventas > 0 ", "", $query_aux);
		...
		if($vista == "actual" || $vista == "resumen" || $vista == "facturacion_detalle"){
			...
			if($vista == "facturacion_detalle"){
				$json_string .= "\"Local\",\"Min $\",\"m2\",\"Min $ / m2\",\"Venta\",
						\"Arr %\",\"Meses\",\"Arr Prom Mensual\",\"Facturacion Total\",\"Facturacion Total \",\"Arr Total\",\"Arr Total / Vta (%)\",\"Venta \",
						\"Arr % \",\"Meses \",\"Arr Prom Mensual \",\"Arr Total \",\"Arr Total / Vta (%) \",
						\"Vta\",
						\" Arr Total\",
						\"Arr Prom / m2\",
						\"Facturacion Total / m2\",
						\"Arr Prom Mensual \",
						\"Vta/m2\",
						\"Facturacion / Vta (%) \",\" Facturacion\",\" Facturacion / Vta (%) \",\"Fondo Prom\"
						,\"Gastos Comunes\",\"Otros\",\"Pago Cta\",\"Fondo Prom  \",\"Gastos Comunes  \"
						,\"Otros  \",\"Pago Cta  \",\"Facturacion Total / m2 \",\"Arr Porcentual\",\"Arr Porcentual  \",\"Arr Minimo\",\"Arr Minimo  \",\"Venta  \",\"Facturacion / Vta(%)\"],";
				
			} else {
				$json_string .= "\"Local\",\"Min $\",\"m2\",\"Min $ / m2\",\"Venta\",
						\"Arr %\",\"Meses\",\"Arr Prom Mensual\",\"Facturacion Mensual\",\"Facturacion\",\"Arr Total\",\"Arr Total / Vta (%)\",\"Venta \",
						\"Arr % \",\"Meses \",\"Arr Prom Mensual \",\"Arr Total \",\"Arr Total / Vta (%) \",
						\"Vta\",
						\" Arr Total\",
						\"Arr Prom / m2\",
						\"Facturacion Mensual / m2\",
						\"Arr Prom Mensual \",
						\"Vta/m2\",
						\"Facturacion / Vta (%) \",\" Facturacion\",\" Facturacion / Vta (%) \"],";
			...
			$total['ventas']			= 0;
			$total['ventas_mes']		= 0;
			$total['facturacion_vc_mes']= 0;
			$total['arr_ant']			= 0;
			$total['arr_porcent_ant']	= 0;
			$total['ventas_ant']		= 0;	
			$total['arr_min']			= 0;
			$total['arr_min_m2']		= 0;	
			$total['acmetros']			= 0;	
			$total['arr_tot_mes_ant']	= 0;
			$total['arr_tot_mes']		= 0;
			$total['arr_prom_m2']		= 0;
			$total['vc_m2']				= 0;
			$total['facturacion'] 		= 0;
			$total['facturacion_ant'] 	= 0;
			$total['fac_mes'] 			= 0;
			$total['fac_mes_m2'] 		= 0;
			$total['fac_m2'] 			= 0;
			$total['facturacion_vc'] 	= 0;
			$total['fondo_prom'] 		= 0;
			$total['gastos_com'] 		= 0;
			$total['otros'] 			= 0;
			$total['pago_cta'] 			= 0;
			$total['fondo_prom_mes'] 	= 0;
			$total['gastos_com_mes'] 	= 0;
			$total['otros_mes'] 		= 0;
			$total['pago_cta_mes']		= 0;
			$total['arr_porcentual']	= 0;
			$total['arr_porcentual_mes']= 0;
			$total['arr_minimo'] 		= 0;
			$total['arr_minimo_mes']	= 0;
			...
			while ($row1 = $adb->fetch_array($result)){
				...
				for ($i=0; $i <=count($valores) ; $i++) { 
					...
					$facturacion				=	round($row["facturacion"], 2);	
					$facturacion_ant			=	round($row["facturacion_ant"], 2);	
					$facturacion_vc_ant			=	0;
					$ventas_mes					=	0;
					$facturacion_vc_mes			=	0;
					$fondo_prom					=	round($row["fondo_prom"], 2);	
					$gastos_com					=	round($row["gastos_com"], 2);	
					$otros						=	round($row["otros"], 2);	
					$pago_cta					=	round($row["pago_cta"], 2);	
					$arr_minimo					=	round($row["arr_minimo"], 2);
					$arr_porcentual				=	round($row["arr_porc"], 2);
					...
					if(isset($ventas) && $ventas!=0){
						...
						$facturacion_vc = (double)round(($facturacion / $ventas) * 100, 1);
						$facturacion_vc = number_format($facturacion_vc, 1, ',', '');
					}
					//Filtro los que no tengan datos en Ventas y Arr
					if(($arr>0 || $arr_ant>0) || ($ventas > 0 || $ventas_ant > 0)){

						if(($agrupar == 'rubro' && $i != 0) || ($agrupar != 'rubro' && $i == 0)){
							$total['arr']+=$arr;
						...
						if(isset($ventas_ant) && $ventas_ant!=0){
							...
							$facturacion_vc_ant = (double)round(($facturacion_ant / $ventas_ant) * 100, 1);
							$facturacion_vc_ant = number_format($facturacion_vc_ant, 1, ',', '');
						}
						if(($agrupar == 'rubro' && $i != 0) || ($agrupar != 'rubro' && $i == 0)){
							$total['arr_ant'] 			+= $arr_ant;
							$total['arr_porcent_ant']	+= $arr_porcent_ant;
						...
						if(($agrupar == 'rubro' && $i != 0) || ($agrupar != 'rubro' && $i == 0)){
							$total['arr_min']		+= $arr_min;
							$total['arr_min_m2']	+= $arr_min_m2;
							$total['acmetros']		+= $row['acmetros'];	
							$total['vc_m2']			+= $row['vc_m2'];	
						}

						$arr_tot_mes_ant = 0;
						if(isset($row['arr_porc_ant_cant']) && $row['arr_porc_ant_cant'] != "")	
							$arr_tot_mes_ant = round($arr_ant / $row['arr_porc_ant_cant'], 2);

						
						$arr_tot_mes=0;
						//Cambios por VR
						//if(isset($row['arr_porc_cant']) 
						//	&& $row['arr_porc_cant']!="")	
						//	$arr_tot_mes=round($arr/$row['arr_porc_cant'],2);
						if(isset($cantidad_ar_actual) && $cantidad_ar_actual != '0'){
							$arr_tot_mes 		= round($arr / intval($cantidad_ar_actual), 2);
							$fac_mes 			= round($facturacion / intval($cantidad_ar_actual), 2);
							$fondo_prom_mes 	= round($fondo_prom / intval($cantidad_ar_actual), 2);
							$gastos_com_mes 	= round($gastos_com / intval($cantidad_ar_actual), 2);
							$otros_mes 			= round($otros / intval($cantidad_ar_actual), 2);
							$pago_cta_mes 		= round($pago_cta / intval($cantidad_ar_actual), 2);
							$arr_porcentual_mes = round($arr_porcentual / intval($cantidad_ar_actual), 2);
							$arr_minimo_mes 	= round($arr_minimo / intval($cantidad_ar_actual), 2);
							$ventas_mes			= round($ventas / intval($cantidad_ar_actual), 2);
						}
						if(isset($ventas_mes) && $ventas_mes != '0'){
							$facturacion_vc_mes = (double)round(($fac_mes / $ventas_mes) * 100, 1);
							$facturacion_vc_mes = number_format($facturacion_vc_mes, 1, ',', '');
						}
						...
						if(isset($row['acmetros']) && $row['acmetros'] != ""){	
							...
							$fac_mes_m2 = round($fac_mes / $row['acmetros'], 2);
							$fac_m2 = round($facturacion / $row['acmetros'], 2);
						}
						...
						$total['facturacion'] 		+= $facturacion;
						$total['facturacion_ant'] 	+= $facturacion_ant;
						$total['fac_mes'] 			+= $fac_mes;
						$total['fac_mes_m2'] 		+= $fac_mes_m2;
						$total['fac_m2'] 			+= $fac_m2;
						$total['fondo_prom_mes'] 	+= $fondo_prom_mes;
						$total['gastos_com_mes'] 	+= $gastos_com_mes;
						$total['otros_mes'] 		+= $otros_mes;
						$total['pago_cta_mes']		+= $pago_cta_mes;
						$total['fondo_prom'] 		+= $fondo_prom;
						$total['gastos_com'] 		+= $gastos_com;
						$total['otros'] 			+= $otros;
						$total['pago_cta']			+= $pago_cta;
						$total['arr_porcentual']	+= $arr_porcentual;
						$total['arr_porcentual_mes']+= $arr_porcentual_mes;
						$total['arr_minimo'] 		+= $arr_minimo;
						$total['arr_minimo_mes']	+= $arr_minimo_mes;
						
						$total['facturacion_vc_mes']+= $facturacion_vc_mes;
						$total['ventas_mes'] 		+= $ventas_mes;
			...
			$facturacion =	round($total["facturacion"], 2);	
			$facturacion_ant =	round($total["facturacion_ant"], 2);	
			...
			if($vista == "facturacion_detalle"){
				$json_string .= ",\"".$total["fondo_prom"]."\",
								\"".$total["gastos_com"]."\",
								\"".$total["otros"]."\",
								\"".$total["pago_cta"]."\",
								\"".$total["fondo_prom_mes"]."\",
								\"".$total["gastos_com_mes"]."\",
								\"".$total["otros_mes"]."\",
								\"".$total["pago_cta_mes"]."\",
								\"".$total["fac_m2"]."\",
								\"".$total["arr_porcentual"]."\",
								\"".$total["arr_porcentual_mes"]."\",
								\"".$total["arr_minimo"]."\",
								\"".$total["arr_minimo_mes"]."\"],";
			} else {
				$json_string .= "],";
			}
			...
			$json_string .= "]";
			//$log->debug("JSON ACA: ".$json_string);
		}elseif($vista == "detalle"){
			$log->debug("entra detalle");
			$json_string=$this->procesarVistaDetalle($query,$request,$query_aux);
		}elseif($vista == "facturacion"){
			$log->debug("entra facturacion");
			$json_string=$this->procesarVistaFacturacion($query,$request,$query_aux);
		}else{
			$log->debug("no entra detalle");
	...
	public function procesarVistaFacturacion($query,$request,$query_aux){
		$adb = PearDatabase::getInstance();
	...
	public function procesarVistaVentas($query,$request,$query_aux){
		...
		//$log->debug("Meses: ".var_export($meses, true));
		//$log->debug("Total: ".var_export($total, true));
