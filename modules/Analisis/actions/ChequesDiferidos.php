...
/*		ini_set("display_errors", 1);
	...
	public function getQuery(Vtiger_Request $request){
		...
		$query.=" SELECT s.*,ac.accountid,ac.accountname,ac.acrazsocial,ac.accodigo, DATE_FORMAT(fec_venc, '%d/%m/%Y') as fecha, ac.luc FROM v_lp_saldoscontables s inner join vtiger_account ac on s.num_Cont=ac.acnumerocontrato WHERE 1=1 ";
...
/*		ini_set("display_errors", 1);
		...
		$indice="";
			...
			while ($row = $adb->fetch_array($result)){
				...
				$total_loc[$row["accountid"]][1]+=$row["saldopesos"];
			}
			$local_ant="";
			$total_loc_aux="";
			$cant_local=0;
