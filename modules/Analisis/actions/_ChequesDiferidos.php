...
class Analisis_ChequesDiferidos_Action extends Vtiger_BasicAjax_Action{
	...
	public function getQuery(Vtiger_Request $request){
		...
		$query.=" SELECT s.*,ac.accountid,ac.accountname, DATE_FORMAT(fec_venc, '%d/%m/%Y') as fecha, ac.luc FROM lp_saldoscontables s inner join vtiger_account ac on s.num_Cont=ac.acnumerocontrato WHERE 1=1 ";
