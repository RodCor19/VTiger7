...
class Analisis_Contratos_Action extends Vtiger_BasicAjax_Action{
	...
	public function getQuery(Vtiger_Request $request){
		...
		if($vista=="actual" || $vista=="resumen"){
			...
			$query="SELECT accountid, accountname ,fecha,fecha_prox,metros as acmetros,";
			...
			if($vista=="actual"){
				...
				$query.="SUM(IF(ar.Aniomes>='".$fecha_desde_anterior."' AND ar.Aniomes <= '".$fecha_hasta_anterior."' AND cod_grupocon='ARRPORCENTUAL' AND ar.AnioMes >=CONCAT(YEAR(fecha),LPAD(MONTH(fecha),2,'0')) 
					...
					(select COUNT(DISTINCT AnioMes)  FROM lp_arrendamientos ar WHERE ar.numcont_lin = ac.acnumerocontrato AND ar.Aniomes >= '".$fecha_desde_anterior."' AND ar.Aniomes <= '".$fecha_hasta_anterior."' AND (cod_grupocon='ARRPORCENTUAL' )) AS arr_porc_ant_cant";*/
			...
			AND s.fecha < s1.fecha LIMIT 1),NOW()) AS fecha_prox
			...
			if($vista=="detalle"){
				...
				$query="SELECT ac.accountid ,ac.accountname, ar.ImpSIva AS arr_porc ,
						AnioMes ,acnumerocontrato,cod_grupocon
						FROM lp_arrendamientos ar 
				...
				$query.=" GROUP BY ac.acnumerocontrato, ar.AnioMes, cod_grupocon
				...
				$query="SELECT ac.accountid ,ac.accountname, vc.VentasSIva AS ventas ,
						...
						INNER JOIN vtiger_account ac ON vc.Contrato = ac.acnumerocontrato
						inner join vtiger_crmentity e on e.crmid=ac.accountid 
						WHERE deleted=0 ";
				...
				$query.=" GROUP BY ac.acnumerocontrato, vc.AnioMes
	...
	public function procesarVistaDetalle($query,$request){
		...
		foreach ($datos as $local=>$mes) {
			...
			$local_url="<a href='index.php?module=Accounts&view=Detail&record=".$locales[$local]."'>".$local."</a>";
