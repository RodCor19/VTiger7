...
//ac.acarminimo as arr_min,
			$query.=" FROM(
			SELECT lpfamilia,lprubro,s.metros,ac.accountid,superficieid,ac.acnumerocontrato,s.supnomcuenta AS accountname,s.fecha, IFNULL((SELECT s1.fecha FROM vtiger_superficie s1 WHERE s.cuenta=s1.cuenta
			...
			if($vista=="detalle"){
				...
				$query.="SELECT lpfamilia,lprubro,ac.accountid ,ac.accountname, ar.ImpSIva AS arr_porc ,
						...
						FROM lp_arrendamientos ar 
						INNER JOIN (SELECT lpfamilia,lprubro,s.metros,ac.accountid,superficieid,ac.acnumerocontrato,s.supnomcuenta as accountname,s.fecha, 
			...
			while ($row1 = $adb->fetch_array($result)){
				...
				for ($i=0; $i <=count($valores) ; $i++) { 
					...
					if(($arr>0 || $arr_ant>0) || ($ventas > 0 || $ventas_ant > 0)){
						...
						$arr_tot_mes_ant=0;
						if(isset($row['arr_porc_ant_cant']) 
