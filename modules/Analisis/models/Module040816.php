	...
	public function getVentilacionClientes(Vtiger_Request $request) {
		...
		LOS 1, WTC Member,COMPRADOR FRECUENTE*/
		 ...
		 $query="
			...
			SUM(IF(programa LIKE '%Mis Beneficios%',1,0 )) AS funcionarios,
			...
			SUM(IF(programa LIKE '%WTC Member%' AND programa LIKE '%Mis Beneficios%' ,1,0 )) AS wtc_funcionarios,
			...
			SUM(IF(programa LIKE '%Comprador Frecuente%' AND programa LIKE '%Mis Beneficios%' ,1,0 )) AS frecuentes_funcionarios,
			...
			SUM(IF(programa LIKE '%Los 1%' AND programa LIKE '%Mis Beneficios%' ,1,0 )) AS unos_funcionarios,
			...
			SUM(IF(programa LIKE '%Programas Gaviotas%' AND programa LIKE '%Mis Beneficios%' ,1,0 )) AS gaviotas_funcionarios,
			...
			SUM(IF(programa LIKE '%Socio Afin%' AND programa LIKE '%Mis Beneficios%' ,1,0 )) AS afines_funcionarios,
			...
			SUM(IF(programa LIKE '%Socio J&J%' AND programa LIKE '%Mis Beneficios%' ,1,0 )) AS jyjs_funcionarios,
			...
			SUM(IF(programa LIKE '%Administrativo%' AND programa LIKE '%Mis Beneficios%' ,1,0 )) AS administrativos_funcionarios,
			...
			SUM(IF(programa LIKE '%Mis Beneficios%' AND programa LIKE '%Proveedor%' ,1,0 )) AS funcionarios_proveedores,
			SUM(IF(programa LIKE '%Mis Beneficios%' AND programa LIKE '%Club de Golf%' ,1,0 )) AS funcionarios_golfs,
			SUM(IF(programa LIKE '%Mis Beneficios%' AND programa LIKE '%Tarjeta Beneficios%' ,1,0 )) AS funcionarios_beneficios,
			...
			SUM(IF(lppromociones is not null and lppromociones!='NULL' AND programa LIKE '%Mis Beneficios%' ,1,0 )) AS promociones_funcionarios,
		...
		if($no_of_rows>0){
			...
			$json_string="[[\"Programa\",\"Todas Personas\",\"Generico\",\"Comprador Frecuente\",\"Socio Afin\",\"Los 1\",
							\"Socio J&J\",\"Programas Gaviotas\",\"Administrativo\",\"Mis Beneficios\",\"Proveedor\",\"WTC Member\",
			...
			*/
			...
			$json_string.="[\"Mis Beneficios\",\"100%\",\"".$this->getPctje($matriz["genericos"]["funcionarios"],$funcionarios)."\",\"".$this->getPctje($matriz["frecuentes"]["funcionarios"],$funcionarios)."\",
