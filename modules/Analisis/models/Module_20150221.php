...
class Analisis_Module_Model extends Vtiger_Module_Model {
	...
	while ($row = $adb->fetch_array($result)){
		...
		$activasdia=$row["activas"]/$dias;
		$reclutadasdia=$row["reclutadas"]/$dias;
		$activasdiaobj=$row["activas_obj"]/$dias;
		$reclutadasdiaobj=$row["reclutadas_obj"]/$dias;
		$clientes=$row["cant_cont"];
	...
	public function getRankingClientes(Vtiger_Request $request) {
		...
		$adb = PearDatabase::getInstance();
                ...
                $query="
		 SELECT firstname, lastname, email, phone, birthday, actuales, anteriores, evolucion
		 FROM (

		 SELECT firstname,lastname,email,phone ,birthday,
                    (SELECT Sum(gvcantidad)
                     FROM   vtiger_gaviotas vtiger_gaviotas1
                     WHERE  vtiger_gaviotas.`gvcontacto` = vtiger_gaviotas1.gvcontacto
                            AND gvfecha >= Date_sub(Now(), INTERVAL 12 month)
                            AND ( gvtipomov = 'Gaviotas generadas en el Canje'
                                   OR gvtipomov = 'Gaviotas generadas por Devolucion'
                                   OR gvtipomov = 'Gaviotas generadas por Boletas' )) AS
                    actuales,
                    (SELECT Sum(gvcantidad)
                     FROM   vtiger_gaviotas vtiger_gaviotas2
                     WHERE  vtiger_gaviotas.`gvcontacto` = vtiger_gaviotas2.gvcontacto
                            AND gvfecha >= Date_sub(Now(), INTERVAL 24 month)
                            AND gvfecha < Date_sub(Now(), INTERVAL 12 month)
                            AND ( gvtipomov = 'Gaviotas generadas en el Canje'
                                   OR gvtipomov = 'Gaviotas generadas por Devolucion'
                                   OR gvtipomov = 'Gaviotas generadas por Boletas' )) AS
		...
		$query.=" GROUP BY contactid";
		switch ($vista) {
			case 'menor':
				$query.=" LIMIT 100 ) calculado ORDER BY evolucion ASC ";				
				break;
			case 'todos':
				$query.=") calculado ORDER BY evolucion ASC";				
				break;
			
			default:
				$query.="  LIMIT 100 ) calculado ORDER BY evolucion DESC ";				
				break;
		}
	...
	public function getClientes(Vtiger_Request $request) {
		...
		if($mostrar_canjes){
				...
				$query="SELECT COUNT(DISTINCT cont) viejos, COUNT(DISTINCT contnuevos) nuevos, nummes
						FROM(
							SELECT IF(MONTH(createdtime)=MONTH(cafecha) ,cacontacto,NULL) AS contnuevos, 
							IF(MONTH(createdtime)<>MONTH(cafecha) ,cacontacto,NULL) 
							AS cont, MONTH(cafecha) AS nummes
							FROM vtiger_canjes
							INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_canjes.cacontacto";
							if($filtrar){
					$query.=" INNER JOIN vtiger_contactdetails on vtiger_contactdetails.contactid=vtiger_canjes.cacontacto ";
				}
			...
			if($mostrar_gaviotas){
					$query.="
					SELECT IF(MONTH(createdtime)=MONTH(gvfecha) ,gvcontacto,NULL) AS contnuevos,
					IF(MONTH(createdtime)<>MONTH(gvfecha) ,gvcontacto,NULL) AS cont, MONTH(gvfecha) AS nummes
					FROM vtiger_gaviotas
					INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=gvcontacto";
					if($filtrar){
						$query.=" INNER JOIN vtiger_contactdetails on vtiger_contactdetails.contactid=vtiger_gaviotas.gvcontacto ";
					}
			...
			if($mostrar_vales){
					$query.=" 
					SELECT IF(MONTH(createdtime)=MONTH(vafecha) ,vacontacto,NULL) AS contnuevos,
					IF(MONTH(createdtime)<>MONTH(vafecha)  ,vacontacto,NULL) AS cont, MONTH(vafecha) AS nummes
					FROM vtiger_vales
					INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vacontacto";
					if($filtrar){
						$query.=" INNER JOIN vtiger_contactdetails on vtiger_contactdetails.contactid=vtiger_vales.vacontacto ";
					}
		...
		if($no_of_rows!=0){
		  $total=0;
		  while($row = $adb->fetch_array($result)){
		        $nombremes=nombremes($row['nummes']);
