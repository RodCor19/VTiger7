...
class Analisis_Module_Model extends Vtiger_Module_Model {
	...
	public function getAnalisisCampania(Vtiger_Request $request) {
		...
		$query="select *,
				...
				AS cant_cont,
				(SELECT  COUNT(DISTINCT gvcontacto) AS cantidad FROM vtiger_contactdetails
				LEFT JOIN vtiger_gaviotas ON vtiger_contactdetails.contactid=gvcontacto		
				WHERE 1=1 ".$filtros." 
				AND ( 
						(gvfecha>= c.lpcamdesde AND gvfecha<=c.closingdate)
						OR  
						contactid IN(
	...
	public function getRankingClientes(Vtiger_Request $request) {
        ...
        */
        $aColumns = array( 'contactid','firstname','lastname','email','phone' ,'birthday', 'cnsaldoactual', 'cnsaldoanterior', 'cndiferencia', 'contactid');
