	...
	public function getClientes(Vtiger_Request $request) {
		...
		if( (!empty($rango) && $rango!="") || (!empty($sexo) && $sexo!="") || (!empty($canal) && $canal!="") || (!empty($estatuto) && $estatuto!="") || (!empty($programa) && $programa!="")   ) {
			...
			$query="SELECT anio,mes, COUNT(contacto) AS total, 
					...
					FROM (";
		if($mostrar_canjes){				
			$query.="SELECT distinct YEAR(cafecha) AS anio, MONTH(cafecha) AS mes ,cacontacto AS contacto
						FROM vtiger_canjes";
		...
		if($mostrar_gaviotas){
				$query.="
				SELECT distinct YEAR(gvfecha) AS anio,MONTH(gvfecha) AS mes , gvcontacto AS contacto
		...
		if($mostrar_vales){
				$query.=" 
				SELECT distinct YEAR(vafecha) AS anio,MONTH(vafecha) AS mes ,vacontacto AS contacto
