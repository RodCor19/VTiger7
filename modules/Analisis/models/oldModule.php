...
/*M E*/		}	
	...
	public function getAnalisisCampania(Vtiger_Request $request) {
		...
		$query="select *,
				...
				AS cant_cont,
				(SELECT  COUNT(DISTINCT contactid) AS cantidad FROM vtiger_contactdetails
				WHERE 1=1 ".$filtros." 
				AND ( 
						contactid IN (SELECT DISTINCT gvcontacto FROM vtiger_gaviotas i
						WHERE gvfecha>=  c.lpcamdesde AND gvfecha<=c.closingdate)
						OR  
						contactid IN(
								SELECT DISTINCT contactid 
								FROM vtiger_crmentity cr INNER JOIN vtiger_contactdetails c ON c.contactid=cr.crmid 
								WHERE c.canal_activo LIKE '%ESHOP%' and createdtime>=  c.lpcamdesde AND createdtime<=c.closingdate
						)
					)
				...
				(SELECT COUNT( crmid ) AS cant FROM vtiger_crmentity 
					...
					".$filtros." 
					AND (
					 	crmid IN (SELECT DISTINCT gvcontacto FROM vtiger_gaviotas i
						WHERE gvfecha>=  c.lpcamdesde AND gvfecha<=c.closingdate)
						OR 
						crmid IN(
								SELECT DISTINCT contactid 
				...
				(SELECT  COUNT(DISTINCT contactid) AS cantidad FROM vtiger_contactdetails
				 LEFT JOIN vtiger_gaviotas ON vtiger_contactdetails.contactid=gvcontacto			
				WHERE gvfecha>= c.lpcamdesde AND gvfecha<=c.closingdate
				".$filtros." 
				and (
						contactid in (
							select contactid from vtiger_campaigncontrel 
				...
				(SELECT COUNT( crmid ) AS cant 
					...
					AND (
						crmid IN (
							SELECT DISTINCT gvcontacto FROM vtiger_gaviotas i 
							WHERE gvfecha>=  c.lpcamdesde 
				...
				(SELECT SUM(TlkRVMontoNeto) AS total FROM lp_ventas_rubro
				WHERE TlkFecha>=  c.lpcamdesde 
							AND TlkFecha<=c.closingdate ) as consumo
		...
		$json_string="[[\"Campaña\",\"Desde\",\"Hasta\",\"SMS Enviados\",\"Mensajes Enviados\",
						...
						\"Clientes reclutados por día\",\"USD por dia\",\"USD por cliente\",
						\"Clientes activos por día \",
						\"Clientes reclutados por día \",\"USD por dia \",\"USD por cliente \",\"Cantidad de Clientes\"],";
