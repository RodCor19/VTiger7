...
function saveExcel(id,table,text){
    ...
    filtrosHtml=exportarFiltros(text);
...
function exportarFiltros(name){
      var tablaFiltros=jQuery("#tablaFiltros").clone();
      var cabezal=jQuery("<div>").append("<b>"+name+"</b><br><div id='filtros'>").html();
      var retorno="";
      if(typeof(filtrosExcel) != "undefined" && filtrosExcel !== null) {
          
        $('.detailViewInfo input,.detailViewInfo  select').each(
          function(index){  
              var input = $(this);
              var key=Object.keys(filtrosExcel).indexOf(input.attr('name'));
              if(key>=0){
                 var valor=input.val();
                 if(input.is('select') === true){
                    valor=input.find('option:selected').text();
                 }
                 if(valor!="" && valor!="Todo" && valor!="Todos"){
                  var row = $('<tr></tr>');
                   var rowData=$('<td></td>').text(filtrosExcel[input.attr('name')]); 
                   row.append(rowData);   
                   rowData=$('<td></td>').text(valor); 
                   row.append(rowData);   
                   tablaFiltros.children('tbody').append(row); 
                 }
        ...
        var html2=jQuery("<table>").append( "<table>"+tablaFiltros.html()+"</table>").html();
        retorno=cabezal+html2;
      }
      //console.log(tablaFiltros.html());
      //var html2=jQuery("<div>").append( jQuery("#tablaFiltros").eq(0).clone()).html();
...
function tableToExcel(table,name,esClase){
  ...
  if(ejecutar){
    ...
    if(window.location.search.indexOf("Quejas") > 0){
		var nuevaData="";
		var dataPartida = html.split("<a href=");
		for (var i in dataPartida){
			//console.log("i: "+i+" data: "+dataPartida[i]);
			var asiQueda = dataPartida[i];
			if(i > 0){
				//asiQueda = "=hyperlink("+dataPartida[i].replace(">",";\"").replace("</a>","\")");
				//no funciono la parte de arriba, sacamos nomas los href
				asiQueda = dataPartida[i].substr(dataPartida[i].indexOf(">") +1).replace("</a>","");
			}
			nuevaData += asiQueda;
		}
		if(nuevaData) html = nuevaData;
	}
	
	  //obtener filtros
	  filtrosHtml=exportarFiltros(name);
	  html=filtrosHtml+html;
	  //console.log(html);
	  ...
	  while (html.indexOf('ñ') != -1) html = html.replace('ñ', '&ntilde;'); 
	  while (html.indexOf('Ñ') != -1) html = html.replace('Ñ', '&Ntilde;');  
	  console.log("data:");console.log(html);
